...
class TournamentGUI:
    ...
    def setup_styles(self):
        ...
        style.theme_use("clam")  # Use a clean baseline
    ...
    # Optional: dim disabled options instead of flashing
        style.map("TCheckbutton",
    ...
    def setup_round_screen(self):
        ...
        ttk.Button(control_frame, text="Submit Round Results", command=self.submit_results).pack(side="left", padx=5)
        ttk.Button(control_frame, text="View Standings", command=self.view_standings).pack(side="left", padx=5)
        ttk.Button(control_frame, text="Undo Last Round", command=self.undo_last_round).pack(side="left", padx=5)
        ttk.Button(control_frame, text="➕ Add Late Player", command=self.add_late_player).pack(side="left", padx=5)
